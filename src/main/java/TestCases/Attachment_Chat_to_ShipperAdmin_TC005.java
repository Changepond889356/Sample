...
public class Attachment_Chat_to_ShipperAdmin_TC005 extends SetUp {
	
	@Test(dataProvider = "getData")
	public void chatFeatures(String sAcccountType, String sName, String sUserName, String sPassword) throws Exception {
		
		String sTestCaseID = "Attachment_Chat_to_ShipperAdmin_TC005";
		...
		try {
			...
			if (bResult == true) {				
				//Chat Invite 
				bResult = Chat.ChatValidation(sTestCaseID,sName, 64); 				
			}
			if(bResult==true)
			{
				if (bResult == true) {	
					//send image
					bResult = Chat.ChatValidation(sTestCaseID,sName, 65); 
					...
					if (bResult == true) {				
						...
						bResult = Chat.ChatValidation(sTestCaseID,sName, 66);  
						if(bResult) {
