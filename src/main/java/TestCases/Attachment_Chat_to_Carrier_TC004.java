...
public class Attachment_Chat_to_Carrier_TC004
 ...
 extends SetUp {
	...
	public void chatFeatures(String sAcccountType, String sName, String sUserName, String sPassword) throws Exception {
		...
		String sTestCaseID = "Attachment_Chat_to_Carrier_TC004";
		...
		try {
			...
			if (bResult == true) {				
				//Chat Invite 
				bResult = Chat.ChatValidation(sTestCaseID,sName, 42); 				
			}
			if(bResult==true)
			{
				if (bResult == true) {	
					//send image
					bResult = Chat.ChatValidation(sTestCaseID,sName, 43); 
					...
					if (bResult == true) {				
						...
						bResult = Chat.ChatValidation(sTestCaseID,sName, 44);  
						if(bResult) {
