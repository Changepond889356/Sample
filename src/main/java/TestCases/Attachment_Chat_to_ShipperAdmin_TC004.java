...
public class Attachment_Chat_to_ShipperAdmin_TC004 extends SetUp {
	
	@Test(dataProvider = "getData")
	public void chatFeatures(String sAcccountType, String sName, String sUserName, String sPassword) throws Exception {
		
		String sTestCaseID = "Attachment_Chat_to_ShipperAdmin_TC004";
		...
		try {
			...
			if (bResult == true) {				
				//Chat Invite 
				bResult = Chat.ChatValidation(sTestCaseID,sName, 61); 				
			}
			if(bResult==true)
			{
				if (bResult == true) {	
					//send image
					bResult = Chat.ChatValidation(sTestCaseID,sName, 62); 
					...
					if (bResult == true) {				
						...
						bResult = Chat.ChatValidation(sTestCaseID,sName, 63);  
						if(bResult) {
							//bResult = TestActions.LogOut();
							TestActions.CloseApplication();
							sActualResult = "Image sent successfully";
						}
					}
				}
