...
public class Leave_Chat_With_ShipperAdmin_TC006 extends SetUp {
	
	@Test(dataProvider = "getData")
	public void chatFeatures(String sAcccountType, String sName, String sUserName, String sPassword) throws Exception {
		
		String sTestCaseID = "Leave_Chat_With_ShipperAdmin_TC006";
		...
		try {
			...
			if (bResult == true) {				
				//Chat Invite 
				bResult = Chat.ChatValidation(sTestCaseID,sName, 67); 				
			}
			if(bResult==true)
			{
				if (bResult == true) {	
					//send message
					bResult = Chat.ChatValidation(sTestCaseID,sName, 68); 
					...
					if (bResult == true) {				
						...
						bResult = Chat.ChatValidation(sTestCaseID,sName, 69);  
						if(bResult==true) {
							//leave chat
							bResult = Chat.ChatValidation(sTestCaseID,sName, 70);  
							if(bResult==true)
							{
