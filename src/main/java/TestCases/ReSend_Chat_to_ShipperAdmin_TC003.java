...
public class ReSend_Chat_to_ShipperAdmin_TC003 extends SetUp {
	...
	public void chatFeatures(String sAcccountType, String sName, String sUserName, String sPassword) throws Exception {
		...
		try {
			...
			if (bResult == true) {
				// Chat Invite
				bResult = Chat.ChatValidation(sTestCaseID, sName, 58);

				if (bResult == true) {
					// Send Message
					bResult = Chat.ChatValidation(sTestCaseID, sName, 59);
					if (bResult==true) {
