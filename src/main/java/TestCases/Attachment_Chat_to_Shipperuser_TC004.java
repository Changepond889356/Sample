...
public class Attachment_Chat_to_Shipperuser_TC004 extends SetUp {
	
	@Test(dataProvider = "getData")
	public void chatFeatures(String sAcccountType, String sName, String sUserName, String sPassword) throws Exception {
		
		String sTestCaseID = "Attachment_Chat_to_Shipperuser_TC004";
		...
		try {
			...
			if (bResult == true) {				
				//Chat Invite 
				bResult = Chat.ChatValidation(sTestCaseID,sName, 80); 				
			}
			if(bResult==true)
			{
				if (bResult == true) {	
					//send image
					bResult = Chat.ChatValidation(sTestCaseID,sName, 81); 
					...
					if (bResult == true) {				
						...
						bResult = Chat.ChatValidation(sTestCaseID,sName, 82);  
						if(bResult) {
