...
public class Leave_Chat_With_ShipperUser_TC006 extends SetUp {
	...
	public void chatFeatures(String sAcccountType, String sName, String sUserName, String sPassword) throws Exception {
		...
		try {
			...
			if (bResult == true) {				
				//Chat Invite 
				bResult = Chat.ChatValidation(sTestCaseID,sName, 86); 				
			}
			if(bResult==true)
			{
				if (bResult == true) {	
					//send message
					bResult = Chat.ChatValidation(sTestCaseID,sName, 87); 
					...
					if (bResult == true) {				
						...
						bResult = Chat.ChatValidation(sTestCaseID,sName, 88);  
						if(bResult==true) {
							//leave chat
							bResult = Chat.ChatValidation(sTestCaseID,sName, 89);  
							if(bResult==true)
							{
