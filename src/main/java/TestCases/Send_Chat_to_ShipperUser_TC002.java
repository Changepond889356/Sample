...
public class Send_Chat_to_ShipperUser_TC002 extends SetUp {
	...
	public void chatFeatures(String sAcccountType, String sName, String sUserName, String sPassword) throws Exception {
		...
		try {
			...
			if (bResult == true) {
				// Chat Invite
				bResult = Chat.ChatValidation(sTestCaseID, sName, 74);

				if (bResult == true) {
					// Send Message
					bResult = Chat.ChatValidation(sTestCaseID, sName, 75);
					if (bResult==true) {
			...
			if (bResult == true) {
				// Verify Message
				bResult = Chat.ChatValidation(sTestCaseID, sName, 76);
