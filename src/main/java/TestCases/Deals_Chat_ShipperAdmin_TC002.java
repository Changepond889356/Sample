...
public class Deals_Chat_ShipperAdmin_TC002 extends SetUp {

	@Test
	public void ShipperUserChat() throws Exception {
		String sTestCaseID = "Deals_Chat_ShipperAdmin_TC002";
			...
			if (bResult == true) {
				...
				if (bResult == true) {
					...
					if (bResult == true) {
						bResult = false;
						// check shared deal
						bResult = Deals.ChatDeal(sTestCaseID,3);
						if (bResult == true) {
							TestActions.LogOut();
						}
						bResult = TestActions.Login("Deals_Chat_ShipperAdmin_TC002(2)");
						if (bResult == true) {
							bResult = Deals.ChatDeal(sTestCaseID,4);
						}
						if (bResult == true) {
							TestActions.LogOut();
